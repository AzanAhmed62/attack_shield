...
class SettingsScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // API keys
    final geminiCtrl   = useTextEditingController(text: GeminiService().apiKey   ?? '');
    final otxCtrl      = useTextEditingController(text: OsintService().otxKey    ?? '');
    final abuseCtrl    = useTextEditingController(text: OsintService().abuseIpKey ?? '');
    final vtCtrl       = useTextEditingController(text: OsintService().virusTotalKey ?? '');
    final showKeys     = useState(false);

    final geminiStatus = useState<String?>(null);
    final otxStatus    = useState<String?>(null);
    ...
    Future<void> testGeminiKey() async {
      ...
      final result = await GeminiService().generate(prompt: 'Reply: OK', maxTokens: 10);
    ...
    Future<void> saveOsintKeys() async {
      ...
      if (otxCtrl.text.trim().isNotEmpty)   await osint.setOtxKey(otxCtrl.text.trim());
      if (abuseCtrl.text.trim().isNotEmpty) await osint.setAbuseIpKey(abuseCtrl.text.trim());
      if (vtCtrl.text.trim().isNotEmpty)    await osint.setVirusTotalKey(vtCtrl.text.trim());
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    Future<void> clearCache() async {
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text('ATT\u0026CK data will be re-parsed on next launch (a few seconds).'),
          actions: [
      ...
      if (confirmed == true) {
        final repo = ref.read(attackTechniqueRepositoryProvider) as AttackTechniqueRepositoryImpl;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _Card(children: [
            _EditTile(icon: Icons.business_outlined, label: 'Organisation',
              value: orgName.value.isEmpty ? 'Tap to set' : orgName.value,
              onTap: () => _editDialog(context, 'Organisation Name', orgName.value, (v) async {
                await storage.write('org_name', v); orgName.value = v;
              })),
            const Divider(height: 1, indent: 48),
            _EditTile(icon: Icons.category_outlined, label: 'Industry Sector',
              value: sector.value.isEmpty ? 'Tap to set' : sector.value,
              onTap: () => _editDialog(context, 'Industry Sector', sector.value, (v) async {
                await storage.write('org_sector', v); sector.value = v;
              })),
          ]),
          const SizedBox(height: 20),

          // ── Gemini AI ──────────────────────────────────────────────────
          _SectionLabel('AI Settings — Gemini'),
          _InfoBanner(
          ...
          // ── OSINT APIs ─────────────────────────────────────────────────
          _SectionLabel('OSINT Intelligence APIs'),
          _InfoBanner(
          ...
          _Card(children: [
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _OsintKeyField(label: 'AlienVault OTX Key', hint: 'From otx.alienvault.com', controller: otxCtrl, show: showKeys.value),
                const SizedBox(height: 10),
                _OsintKeyField(label: 'AbuseIPDB Key',      hint: 'From abuseipdb.com/api',  controller: abuseCtrl, show: showKeys.value),
                const SizedBox(height: 10),
                _OsintKeyField(label: 'VirusTotal Key',     hint: 'From virustotal.com',      controller: vtCtrl, show: showKeys.value),
          ...
          // ── Data & Cache ────────────────────────────────────────────────
          _SectionLabel('Data & Cache'),
          _Card(children: [
            _ActionTile(icon: Icons.cleaning_services_outlined,
              label:   'Clear STIX & OSINT Cache',
              subtitle: 'Force re-parse of ATT\u0026CK data + reset session OSINT cache',
              onTap: clearCache),
            const Divider(height: 1, indent: 48),
            _ActionTile(icon: Icons.refresh_rounded,
          ...
          _SectionLabel('About'),
          _Card(children: [
            _InfoTile(icon: Icons.shield_rounded,       label: 'App',          value: 'ATT\u0026CK Shield'),
            const Divider(height: 1, indent: 48),
            _InfoTile(icon: Icons.dataset_outlined,     label: 'MITRE Data',   value: 'Enterprise ATT\u0026CK v14.5'),
            const Divider(height: 1, indent: 48),
            _InfoTile(icon: Icons.auto_awesome_rounded, label: 'AI Engine',    value: 'Gemini 1.5 Flash'),
            const Divider(height: 1, indent: 48),
            _InfoTile(icon: Icons.radar_rounded,        label: 'OSINT Sources',value: 'OTX · NVD · AbuseIPDB · VT'),
            const Divider(height: 1, indent: 48),
            _InfoTile(icon: Icons.code_rounded,         label: 'Framework',    value: 'Flutter + Riverpod'),
          ]),
        ],
      ),
    );
  ...
  Future<void> _editDialog(BuildContext context, String label, String current,
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(text, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
        color: Theme.of(context).colorScheme.primary, letterSpacing: .4)));
}
...
class _EditTile extends StatelessWidget {
  ...
  const _EditTile({required this.icon, required this.label, required this.value, this.onTap});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
...
class _ApiKeyTile extends StatelessWidget {
  ...
  final bool visible, isTesting; final String? status;
  final VoidCallback onToggle, onSave, onTest;
  const _ApiKeyTile({required this.label, required this.controller,
    required this.visible, required this.isTesting, required this.status,
    required this.onToggle, required this.onSave, required this.onTest});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: cs.onSurface)),
        const SizedBox(height: 8),
        TextField(controller: controller, obscureText: !visible,
          style: const TextStyle(fontSize: 13, fontFamily: 'monospace'),
          decoration: InputDecoration(
            hintText: 'API key...',
            border: const OutlineInputBorder(), isDense: true,
            suffixIcon: IconButton(icon: Icon(visible
                ? Icons.visibility_off_outlined : Icons.visibility_outlined, size: 18),
              onPressed: onToggle))),
        const SizedBox(height: 10),
        Row(children: [
        ...
        if (status != null) ...[
          const SizedBox(height: 8),
          Row(children: [
            Icon(status == 'valid' ? Icons.check_circle_outline_rounded
                : status == 'saved' ? Icons.save_rounded : Icons.error_outline_rounded,
              size: 14, color: status == 'invalid' ? cs.error : Colors.green.shade600),
            const SizedBox(width: 6),
            Text(status == 'valid'   ? 'Key is working ✓'
               : status == 'saved'   ? 'Saved. Tap Test to verify.'
...
class _OsintKeyField extends StatelessWidget {
  final String label, hint; final TextEditingController controller; final bool show;
  const _OsintKeyField({required this.label, required this.hint,
      required this.controller, required this.show});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(controller: controller, obscureText: !show,
