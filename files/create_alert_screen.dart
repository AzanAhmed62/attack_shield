...
// FULL REPLACEMENT — uses AlertItem (existing model) with AlertPriority enum.
// Technique link via dropdown from real STIX data.
...
class CreateAlertScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final priority   = useState(AlertPriority.medium);
    final linkedTech = useState<String?>(null);
    final isSaving   = useState(false);
    final formKey    = useMemoized(GlobalKey<FormState>.new);
    final allTechs   = ref.watch(allTechniquesProvider);
    ...
    Future<void> save() async {
      ...
      isSaving.value = true;
      try {
        final alert = AlertItemFactory.create(
          id:                const Uuid().v4(),
          title:             titleCtrl.text.trim(),
          description:       descCtrl.text.trim(),
          severity:          priority.value.name,
          source:            sourceCtrl.text.trim().isEmpty
                               ? 'Manual' : sourceCtrl.text.trim(),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            Wrap(spacing: 8, children: AlertPriority.values.map((p) {
              final sel   = priority.value == p;
              final color = p == AlertPriority.critical ? Colors.red.shade600
                          : p == AlertPriority.high     ? Colors.orange.shade600
                          : p == AlertPriority.medium   ? Colors.amber.shade600
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Source',
                hintText:  'e.g. SIEM, EDR, Firewall, Manual investigation',
                border:    OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 16),
            ...
            // Link to technique
            Text('Link to MITRE Technique (optional)',
            ...
            allTechs.when(
              ...
              data: (techs) {
                ...
                return DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Select a MITRE ATT\u0026CK technique',
                    border:   OutlineInputBorder(),
                  ),
                  isExpanded: true,
                  items: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                hintText:  'Investigation notes, IOCs, context...',
                border:    OutlineInputBorder(),
              ),
            ),
