        ...
        data: (technique) {
          ...
          final hasActiveAlert =
              alertsAsync
                  .whenData(
                    (alerts) => alerts.any(
                      (a) => a.effectiveLinkedTechniqueId == techniqueId,
                    ),
                  )
                  .value ??
              false;
    ...
    try {
      ...
      if (result.isSuccess && result.text != null) {
        try {
          ...
          ai.value = {'simpleExplanation': result.text};
        }
      } else {
        error.value = result.error;
      }
    } catch (e) {
      error.value = 'Could not load AI explanation: $e';
    } finally {
      loading.value = false;
    }
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> load() async {
      ...
      result.value = await ref
          .read(osintServiceProvider)
          .getOtxPulsesForTechnique(techniqueId);
      loading.value = false;
      expanded.value = true;
    }
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'AlienVault OTX — Live Threat Intel',
                    style: TextStyle(
                      fontSize: 13,
            ...
            Padding(
              ...
              child: result.value!.isSuccess
                  ? result.value!.data!.isEmpty
                        ? Text(
                            'No OTX pulses found for $techniqueId.',
                            style: TextStyle(fontSize: 12, color: cs.outline),
                          )
                        : Column(
                            children: result.value!.data!
                                .map(
                                  (pulse) => Padding(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'NVD / CVE — Related Vulnerabilities',
                    style: TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w600,
                      color: cs.onSurface,
          ...
          if (expanded.value && result.value != null) ...[
            ...
            Padding(
              ...
              child: result.value!.isSuccess
                  ? result.value!.data!.isEmpty
                        ? Text(
                            'No CVEs found for "$techniqueName".',
                            style: TextStyle(fontSize: 12, color: cs.outline),
                          )
                        : Column(
                            children: result.value!.data!.map((cve) {
                              final color = cve.cvssScore >= 9
                                  ? Colors.red.shade600
                                  : cve.cvssScore >= 7
...
class _CoverageControl extends StatelessWidget {
  ...
  Future<void> _set(BuildContext ctx, CoverageLevel level) async {
    await ref
        .read(coverageActionsProvider.notifier)
        .setLevel(techniqueId, level);
    if (ctx.mounted)
  ...
  Future<void> _showNotes(BuildContext ctx) async {
    ...
    if (result != null) {
      ...
      await ref
          .read(coverageActionsProvider.notifier)
          .setLevel(techniqueId, current, notes: result);
    }
  }
}
    ...
    return Column(
      ...
      children: [
        ...
        if (technique.dataSources.isNotEmpty) ...[
          const SizedBox(height: 16),
          _Section(
            title: 'Detection Data Sources (${technique.dataSources.length})',
          ...
          child: Column(
            ...
            children: [
              if (technique.created != null)
                _MetaRow('Created', technique.created!.substring(0, 10)),
              if (technique.modified != null)
                _MetaRow('Modified', technique.modified!.substring(0, 10)),
...
class _PlainBody extends StatelessWidget {
  ...
  Widget _buildActions(BuildContext context, ColorScheme cs) {
    final items =
        (aiExplanation?['topThreeActions'] as List?)?.cast<String>() ??
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '— $level priority attention required.',
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(
            _icons[platform] ?? Icons.devices_rounded,
            size: 12,
