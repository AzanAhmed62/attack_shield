...
class OsintService {
  ...
  Future<void> setOtxKey(String key) async =>
      _storage.write(_kOtxKey, key.trim());
  Future<void> setAbuseIpKey(String key) async =>
      _storage.write(_kAbuseIpKey, key.trim());
  Future<void> setVirusTotalKey(String key) async =>
      _storage.write(_kVirusTotalKey, key.trim());
      ...
      if (resp.statusCode == 200) {
        ...
        final pulses = results.map((p) {
          ...
          return OtxPulse(
            ...
            indicatorTypes:
                (p['indicator_type_counts'] as Map?)?.keys
                    .toList()
                    .cast<String>() ??
                [],
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final vulnerabilities =
            (json['vulnerabilities'] as List?)?.cast<Map<String, dynamic>>() ??
            [];
        final cves = vulnerabilities.map((v) {
          final cve = v['cve'] as Map<String, dynamic>? ?? {};
          final id = cve['id'] as String? ?? '';
          final descs =
              (cve['descriptions'] as List?)?.cast<Map<String, dynamic>>() ??
              [];
          final desc =
              descs.firstWhere(
                    (d) => d['lang'] == 'en',
          ...
          final weaknesses =
              (cve['weaknesses'] as List?)?.cast<Map<String, dynamic>>() ?? [];
          final cwes = weaknesses
              .expand(
                (w) =>
                    (w['description'] as List?)
                        ?.cast<Map>()
                        .where((d) => d['lang'] == 'en')
  ...
  Future<OsintResult<IpReputation>> lookupIp(String ip) async {
    ...
    try {
      ...
      final resp = await http
          .get(uri, headers: {'Key': abuseIpKey!, 'Accept': 'application/json'})
          .timeout(_kTimeout);
  ...
  Future<OsintResult<VtResult>> lookupUrl(String url) async {
    ...
    final cacheKey = url;
    if (_vtCache.containsKey(cacheKey))
      return OsintResult.success(_vtCache[cacheKey]!);
    try {
      ...
      final resp = await http
          .get(
            uri,
            headers: {'x-apikey': virusTotalKey!, 'Accept': 'application/json'},
          )
          .timeout(_kTimeout);

      if (resp.statusCode == 200) {
        final attrs =
            ((jsonDecode(resp.body) as Map)['data'] as Map)['attributes']
